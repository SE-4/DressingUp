...
function ajaxCallBack(url,params,success,fail,modal){
	params || (params="{}");
	var root = __APP__;
	if(modal) openBackGround();
	$.ajax({
		async:true,
		type:"post",
		url:root+url,
		...
		success:function(data){
			if(modal) closeBackGround();
			success(data);
		}
	});
}
function uploadFile(fm,callback){
	fm.form('submit', {
	    url: __APP__+"UploadServlet",
	    success: function (data) {
	        closeBackGround();
